...
# basic settings
stage=1      # stage to start, 2: train
...
if [ "${stage}" -le 2 ] && [ "${stop_stage}" -ge 2 ]; then
    ...
    if [ "${n_gpus}" -gt 1 ]; then
        train="CUDA_VISIBLE_DEVICES=2,3 python -m parallel_wavegan.distributed.launch --nproc_per_node ${n_gpus} -c parallel-wavegan-train"
    else
